...
public class NgiiSSOLoginFilter implements Filter{
	
	private FilterConfig config;
	
	protected final static Log LOG = LogFactory.getLog(NgiiSSOLoginFilter.class);
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		NgiiSSOService ngiiSSOService = null;
		try{
			ngiiSSOService = (NgiiSSOService) act.getBean("ngiiSSOService");
			if(ngiiSSOService == null){//2011.10.27 보안점검 3차 후속조치
				LOG.error("Fail to create 'NgiiSSOService' object");
		...
		if(isLocallyAuthenticated != null && (isLocallyAuthenticated.equals("true"))){
			if(isRemotelyAuthenticated == null){
				...
				try {				
					//sso서버에 토큰 생성
					ngiiSSOService.requestIssueToken(request, response);	
			...
			if(isRemotelyAuthenticated == null){
				...
				isSSOLoggedOn = ngiiSSOService.hasTokenInSSOServer(httpRequest, response);
				if(isSSOLoggedOn){	
					//서버에 토큰이 존재할 경우 로컬 인증을 위해 isRemotelyAuthenticated true로 변경
					session.setAttribute("isRemotelyAuthenticated", "true");
					
					//로컬 DB인증을 위한 loginVO 객체를 세션에 저장
					session.setAttribute("loginVOForDBAuthentication", ngiiSSOService.getLoginVO(request, response));
				}
			}
		}
