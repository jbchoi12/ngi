...
public class EgovBBSManageController {
    ...
    @RequestMapping("/cop/bbs/selectBoardArticleJson.do")
    public void selectBoardArticleJson(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model, HttpServletResponse response) throws Exception {
	...
	@RequestMapping("/cop/bbs/selectGuestListJson.do")
	public void selectGuestListJson(
			@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model, HttpServletResponse response)
			throws Exception {
		...
		try {
			Map<String, Object> map_d = new HashMap<String, Object>();
			if (!((java.util.ArrayList) map.get("resultList")).isEmpty()) {
				map_d.put(str_json_data, map.get("resultList"));
				map_d.put(str_total, totCnt);
				map_d.put(str_display, ((java.util.ArrayList) map.get("resultList")).size());
			} else {
				map_d.put(str_json_data, "");
				map_d.put(str_total, 0);
				map_d.put(str_display, 0);
			}
			map_d.put(str_start, boardVO.getPageIndex());
			ObjectMapper om = new ObjectMapper();
			str_resdata = om.writeValueAsString(map_d);
		} catch (Exception ex) {
			//System.out.println(this.getClass().toString() + " Exception : " + ex.toString());
...
//		model.addAttribute("paginationInfo", paginationInfo);
	...
	@RequestMapping("/cop/bbs/selectBoardListJson.do")
	public void selectBoardArticlesJson(
		...
		// -------------------------------
		if (master.getBbsTyCode().equals("BBST04")) {
			selectGuestListJson(boardVO, model, response);
			return;
		}
		// //-----------------------------
...
//		model.addAttribute("paginationInfo", paginationInfo);
	...
	if (isAuthenticated) {
	    ...
	    board.setPssword("");	// dummy 오류 수정 (익명이 아닌 경우 validator 처리를 위해 dummy로 지정됨)
	...
	if (isAuthenticated) {
	    ...
	    board.setPssword(EgovFileScrty.encryptPassword(board.getPssword()));
	...
	String enpassword = EgovFileScrty.encryptPassword(board.getPssword());
	...
	if (isAuthenticated) {
	    ...
	    String enpassword = EgovFileScrty.encryptPassword(boardVO.getPssword());
